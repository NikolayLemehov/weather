...
export const openWeatherApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getCityByCoordinates: builder.query<[GeoCityApiType], CoordType>({
      query: ({ lat, lon }) => `geo/1.0/reverse?lat=${lat}&lon=${lon}&appid=${OPENWEATHER_API_KEY}`,
    }),
...
export const { useSearchCitiesQuery, useGeoWeatherQuery, useLazyGetCityByCoordinatesQuery } = openWeatherApi;
